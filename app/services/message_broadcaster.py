...
class MessageBroadcaster:
    ...
    async def broadcast_task_start(self, task_id: str, task_type: str, title: str, total_items: int = 0):
        message = self._create_message(
            MessageType.TASK_START,
            {"task_id": task_id, "task_type": task_type, "title": title, "total_items": total_items},
        )
    ...
    async def broadcast_task_complete(
        self, task_id: str, task_type: str, result: dict[str, Any], result_message: str = ""
    ):
        message = self._create_message(
            MessageType.TASK_COMPLETE,
            {"task_id": task_id, "task_type": task_type, "result": result, "result_message": result_message},
        )
        await self._manager.broadcast(message)

    async def broadcast_task_error(self, task_id: str, task_type: str, error: str):
        message = self._create_message(
            MessageType.TASK_ERROR, {"task_id": task_id, "task_type": task_type, "error": error}
        )
        await self._manager.broadcast(message)
